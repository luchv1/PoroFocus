...
export default function Task({ title, icon, status, onDone, onDelete, isFocusMode }) {
    ...
    return (
        <div className={`text-left flex mt-2 items-center justify-between
            border rounded-lg p-3 border-neutral space-x-2 cursor-pointer ${blockClasses}`}
            onClick={onDone}>
            ...
            {!isFocusMode && <Button onClick={onDelete} classes="hover:bg-gray-200 hover:scale-120"><X size={16} /></Button>}
        </div>
    )
}
